...
public sealed class ConfigGenerationPipeline
{
    ...
    public OperationResult ValidateTables(List<TableData> tables)
    {
    ...
    public async Task<List<TableData>> LoadAndValidateAsync(List<ISpreadsheetDataSource> spreadsheetSources)
    {
        ...
        return result.ValidationResult.IsSuccess ? result.Tables : new List<TableData>();
    }
    ...
    private async Task<PipelineRunResult> GenerateFromSourcesAsync(
        ...
        Func<List<TableData>, string, OperationResult> generateAction)
    {
    ...
    {
        OperationResult validationResult = ValidateTables(tables);
    ...
    private static PipelineRunResult GenerateFromValidated(
        ...
        Func<List<TableData>, string, OperationResult> generateAction)
    {
        if (!validated.ValidationResult.IsSuccess)
        {
            return CreateValidationFailedRunResult(validated);
        }

        OperationResult generation = generateAction(validated.Tables, outputFolderPath);
    ...
    {
        OperationResult generationResult = new OperationResult(OperationResultIdentifiers.Generator);
        generationResult.AddWarning("Generation skipped because validation failed.", step: OperationSteps.Generation.Artifacts);
